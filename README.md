...
- 判断与计算：定义常量 `IconSpacing=120px`（相邻两磁贴同一角的水平距离，包含磁贴宽与外边距）。按下 `Thumb` 时记录起始水平坐标 `_resizeAnchorX`；拖动中实时取当前鼠标水平坐标求差 `delta=currentX-_resizeAnchorX`，当 |delta| ≥ `IconSpacing*0.75`（约 90px）时触发一次列数调整，方向取决于 delta 正负。触发后立即把 `_resizeAnchorX` 更新为当前坐标，继续同样判定，避免跳列或闪烁；列数下限为 1。
